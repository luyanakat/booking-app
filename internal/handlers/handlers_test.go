...
var theTests = []struct {
	...
	{"normal-room", "/normal-room", "GET", http.StatusOK},
	{"luxurious-room", "/luxurious-room", "GET", http.StatusOK},
	{"contact", "/contact", "GET", http.StatusOK},
	{"search-avai", "/search-availability", "GET", http.StatusOK},
	{"reservation-summary", "/reservation-summary", "GET", http.StatusMethodNotAllowed},
	// POST
...
func TestRepository_Reservation(t *testing.T) {
	reservation := models.Reservation{
		RoomID: 1,
		Room:   models.Room{ID: 1, RoomName: "Normal room"},
	}
	...
	if rr.Code != http.StatusOK {
		t.Errorf("Reservation handler returned wrong respone, "+
...
func TestRepository_PostReservation(t *testing.T) {
	...
	reqBody = fmt.Sprintf("%s&%s", reqBody, "first_name=dam")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "last_name=hoa")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "email=dam@hoa.com")
	...
	// another way to post reqBody
	//reqData := url.Values{}
	//reqData.Add("start_date", "2050-01-02")
	//req, _ = http.NewRequest("POST", "/make-reservation", strings.NewReader(reqData.Encode()))
	//
